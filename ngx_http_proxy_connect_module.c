...
typedef struct {
    ...
    ngx_int_t                           website_access_ctrl;
    ngx_int_t                           website_whitelist_mode;
    ngx_array_t                         *rules;     /* array of ngx_http_access_ctrl_rule_t */
...
static char *ngx_http_access_ctrl_rule(ngx_conf_t *cf, ngx_command_t *cmd, void *conf);
...
ngx_http_proxy_connect_finalize_request(ngx_http_request_t *r,
    ...
    ngx_http_proxy_connect_loc_conf_t *alcf;

    alcf = ngx_http_get_module_loc_conf(r, ngx_http_proxy_connect_module);

    /*
    * 如果没开上网控制，直接放行
    */
    if (alcf->website_access_ctrl) {
        /**
         * 如果开启了白名单模式，则仅限白名单网站可访问；否则为黑名单模式，只有黑名单的网站不能访问，其他都能访问
         */

        ngx_uint_t i;
        ngx_http_access_ctrl_rule_t *rule;
        ngx_str_t host = (ngx_str_t) r->headers_in.host->value;

        rule = alcf->rules->elts;

        if (alcf->website_whitelist_mode) {
            if (!alcf->rules) {
                ngx_http_proxy_connect_finalize_request(r, ctx->u, NGX_HTTP_FORBIDDEN);
                return;
            }
            for (i = 0; i < alcf->rules->nelts; i++) {

                ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                               "access: %08XD %08XD", host, rule[i].website);

                u_char *website = rule[i].website.data;
                if (!rule[i].deny && ngx_strcmp(host.data, website) == 0) {
...
return NGX_OK;
    ...
    *plocal = local;
    ...
    conf->website_access_ctrl = NGX_CONF_UNSET;
    ...
    *msp = ms;
    ...
    if (alcf->rules == NULL) {
        alcf->rules = ngx_array_create(cf->pool, 4, sizeof(ngx_http_access_ctrl_rule_t));
        if (alcf->rules == NULL) {
            return NGX_CONF_ERROR;
        }
    }
